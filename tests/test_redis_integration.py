...
from verdikt.rate_limiter import RedisRateLimiter


@unittest.skipUnless(os.getenv("VERDIKT_TEST_REDIS_URL"), "integration Redis URL is not configured")
class RedisRateLimiterIntegrationTest(unittest.TestCase):
    def test_two_instances_share_an_atomic_limit(self) -> None:
        url = os.environ["VERDIKT_TEST_REDIS_URL"]
        prefix = f"verdikt:test:{uuid.uuid4()}"
