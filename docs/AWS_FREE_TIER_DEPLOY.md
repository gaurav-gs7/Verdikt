...
- Restrict `MCP_GUARD_ALLOWED_CIDR` to your public IP `/32` instead of `0.0.0.0/0`.
...
flowchart LR
    ...
    EC2 --> ECR
    EC2 --> App["GateTrace MCP container"]
...
If the ARN ends with `:root`, create an IAM user/role or use IAM Identity Center before deploying. The scripts intentionally block root-user deployment unless you set `MCP_GUARD_ALLOW_ROOT_AWS=true` for a one-off lab.
...
The token and approval secret are stored in Secrets Manager. Terraform creates the secret containers and IAM bindings; the deploy script writes the values with AWS CLI so live secret strings do not sit in Terraform state. Lambda receives only the secret ARNs and has scoped permission to read them.
...
The Terraform module lives in [../infra/aws/terraform](../infra/aws/terraform). It manages ECR, IAM, security groups, SSM access, and the EC2 host. The Docker container defaults to `MCP_GUARD_MODE=real-mcp`, which runs the official MCP Streamable HTTP server at `/mcp`.
...
```bash
export AWS_REGION=us-east-1
export MCP_GUARD_DOCKER_PLATFORM=linux/amd64
...
curl -s "$URL/healthz"
curl -s "$URL/metrics" -H "Authorization: Bearer $MCP_GUARD_HTTP_BEARER_TOKEN"
```
...
```bash
sudo docker ps
sudo docker logs mcp-guard --tail 100
...
aws cloudformation wait stack-delete-complete \
  ...
  --stack-name "${MCP_GUARD_STACK_NAME:-mcp-guard-free-tier}"
```
...
aws ecr batch-delete-image \
  --region "${AWS_REGION:-us-east-1}" \
  --repository-name "${MCP_GUARD_ECR_REPOSITORY:-mcp-guard}" \
  --image-ids imageTag="${MCP_GUARD_IMAGE_TAG:-latest}"
```
