...
class GuardedOpsRuntime:
    ...
    def call_tool(
        ...
        with self.telemetry.span(
            ...
            request_span.set_attribute("mcp_guard.policy.action", decision.action)
            if allowed:
                if decision.action in {"DRY_RUN_ONLY", "SHADOW_MODE"}:
                    ...
                    try:
                        ...
                        with self.telemetry.span(
                            f"mcp.{tool}",
                            "TOOL",
                            {"tool.name": tool, "mcp_guard.server": server},
                        ) as tool_span:
                            ...
                            tool_span.set_attribute("mcp_guard.content.allowed", inspection.allowed)
                            tool_span.set_attribute("mcp_guard.content.finding_count", len(inspection.findings))
                            tool_span.set_attribute("mcp_guard.content.hash", inspection.content_hash)
