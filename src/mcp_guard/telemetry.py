...
class TraceSpan:
    ...
    def set_policy(self, *, allowed: bool, rule: str, reason: str) -> None:
        self.set_attribute("mcp_guard.policy.allowed", allowed)
        self.set_attribute("mcp_guard.policy.rule", rule)
        self.set_attribute("mcp_guard.policy.reason", reason)
...
class Telemetry:
    ...
    def __init__(self, mode: str | None = None) -> None:
        ...
        if self.mode not in {"console", "otlp"}:
            raise ValueError("MCP_GUARD_TELEMETRY must be disabled, console, or otlp")
        try:
        ...
        self._tracer = provider.get_tracer("mcp_guard", "0.1.0")
