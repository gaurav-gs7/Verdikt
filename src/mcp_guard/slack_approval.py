...
class SlackApprovalWorkflow:
    ...
    def __init__(self, path: Path, authority: ApprovalAuthority) -> None:
        self.webhook_url = os.getenv("MCP_GUARD_SLACK_WEBHOOK_URL", "")
        self.signing_secret = os.getenv("MCP_GUARD_SLACK_SIGNING_SECRET", "")
        self.approvers = {
            value.strip()
            for value in os.getenv("MCP_GUARD_SLACK_APPROVER_IDS", "").split(",")
            if value.strip()
        }
        self.max_pending_per_requester = int(
            os.getenv("MCP_GUARD_SLACK_MAX_PENDING_PER_REQUESTER", "5")
        )
        ...
        *,
        ...
        if not self.enabled:
            raise SlackApprovalError(
                "Slack approvals require MCP_GUARD_SLACK_WEBHOOK_URL, "
                "MCP_GUARD_SLACK_SIGNING_SECRET, and MCP_GUARD_SLACK_APPROVER_IDS"
            )
        now = int(time.time())
