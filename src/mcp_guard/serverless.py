...
POLICY_PATH = Path(os.getenv("MCP_GUARD_POLICY_PATH", "config/policies.yaml"))
NAMESPACE = "GateTrace/Serverless"
...
def gateway_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """API Gateway HTTP API entrypoint for the serverless GateTrace MCP gateway."""
...
def _call_guarded_tool(
    *,
    ...
    if not allowed or decision.risk_level in {"high", "critical"}:
    ...
    def evaluate(self, server: str, tool: str, arguments: dict[str, Any]) -> PolicyDecision:
        with self._lock:
            ...
            if forbidden_key:
                return self._decision(
                    False,
                    f"caller-supplied credential field {forbidden_key!r} is forbidden; upstream credentials must be brokered by GateTrace MCP",
                    "token_passthrough",
                    risk,
                )
...
def _policy_path() -> Path:
    ...
    if not policy_item or "document" not in policy_item:
        return POLICY_PATH
    runtime_path = Path("/tmp/gatetrace-mcp/policies.json")
...
def _evaluation_only_result(
    ...
    return {
        ...
        "tool": tool,
        "arguments": safe_arguments,
        "message": "GateTrace MCP evaluated policy and intentionally skipped execution.",
    }
...
def _api_token() -> str:
    return _secret_from_env("MCP_GUARD_API_TOKEN_SECRET_ARN") or os.getenv("MCP_GUARD_API_TOKEN", "")


def _approval_secret() -> str | None:
    return _secret_from_env("MCP_GUARD_APPROVAL_SECRET_ARN") or os.getenv("MCP_GUARD_APPROVAL_SECRET")


def _audit_signing_secret() -> bytes:
    value = os.getenv("MCP_GUARD_AUDIT_HMAC_SECRET") or _approval_secret()
