...
def gateway_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """API Gateway HTTP API entrypoint for the serverless Verdikt gateway."""
...
def _policy_path() -> Path:
    ...
    if not policy_item or "document" not in policy_item:
        return POLICY_PATH
    runtime_path = Path("/tmp/verdikt/policies.json")
...
def _api_token() -> str:
    return _secret_from_env("VERDIKT_API_TOKEN_SECRET_ARN") or os.getenv("VERDIKT_API_TOKEN", "")


def _approval_secret() -> str | None:
    return _secret_from_env("VERDIKT_APPROVAL_SECRET_ARN") or os.getenv("VERDIKT_APPROVAL_SECRET")


def _audit_signing_secret() -> bytes:
    value = _secret_from_env("VERDIKT_AUDIT_HMAC_SECRET_ARN") or os.getenv(
        "VERDIKT_AUDIT_HMAC_SECRET"
    )
    if not value and os.getenv("VERDIKT_AUDIT_SIGNATURE_REQUIRED", "").lower() in {
        "1",
        "true",
        "yes",
    }:
