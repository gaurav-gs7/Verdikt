...
class AuthConfig:
    ...
    def from_env(cls) -> "AuthConfig":
        return cls(
            ...
            jwt_jwks_url=os.getenv("VERDIKT_JWT_JWKS_URL", ""),
            resource_uri=os.getenv("VERDIKT_RESOURCE_URI", "http://127.0.0.1:8080/mcp"),
            authorization_server=os.getenv("VERDIKT_AUTHORIZATION_SERVER", ""),
            required_scopes=tuple(
                scope for scope in os.getenv("VERDIKT_REQUIRED_SCOPES", "mcp:tools").split() if scope
            ),
            admin_scope=os.getenv("VERDIKT_ADMIN_SCOPE", "mcp:admin"),
            allowed_origins=tuple(
                origin.strip().rstrip("/")
                for origin in os.getenv("VERDIKT_ALLOWED_ORIGINS", "").split(",")
                if origin.strip()
            ),
        )
    ...
    def validate_for_remote_server(self, *, require_auth: bool = False) -> None:
        ...
        if self.jwt_hs256_secret or self.jwt_jwks_url:
            if not self.jwt_issuer:
                raise AuthError("VERDIKT_JWT_ISSUER is required for remote JWT authentication")
            if not self.jwt_audience:
                raise AuthError("VERDIKT_JWT_AUDIENCE is required for remote JWT authentication")
            if not self.authorization_server:
                raise AuthError(
                    "VERDIKT_AUTHORIZATION_SERVER is required for remote JWT authentication"
                )
        parsed_resource = urllib.parse.urlparse(self.resource_uri)
        if not parsed_resource.hostname:
            raise AuthError("VERDIKT_RESOURCE_URI must be an absolute HTTP(S) URI")
        if parsed_resource.scheme != "https" and parsed_resource.hostname not in {"127.0.0.1", "localhost"}:
            raise AuthError("VERDIKT_RESOURCE_URI must use HTTPS except on localhost")
        if parsed_resource.fragment:
            raise AuthError("VERDIKT_RESOURCE_URI must not contain a fragment")
        for name, url in (
            ("VERDIKT_JWT_JWKS_URL", self.jwt_jwks_url),
            ("VERDIKT_AUTHORIZATION_SERVER", self.authorization_server),
        ):
