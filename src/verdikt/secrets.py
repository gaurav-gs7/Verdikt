...
def read_vault_secret(
    ...
    *,
    ...
    address = os.getenv("VERDIKT_VAULT_ADDR", "").strip()
    if not address:
        raise SecretBrokerError("VERDIKT_VAULT_ADDR is required for Vault secrets")
    ...
    token = resolve_configured_secret(
        direct_env="VERDIKT_VAULT_TOKEN",
        aws_secret_env="VERDIKT_VAULT_TOKEN_SECRET_ARN",
        json_key_env="VERDIKT_VAULT_TOKEN_SECRET_JSON_KEY",
        required=True,
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Verdikt/secret-broker",
            "X-Vault-Token": token,
        },
        method="GET",
    )
    namespace = os.getenv("VERDIKT_VAULT_NAMESPACE", "").strip()
    if namespace:
        request.add_header("X-Vault-Namespace", namespace)
    timeout = _positive_float_env("VERDIKT_SECRET_TIMEOUT_SECONDS", 2.0)
...
def _vault_endpoint(address: str, path: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise SecretBrokerError("VERDIKT_VAULT_ADDR must be an absolute HTTP(S) URL")
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise SecretBrokerError(
            "VERDIKT_VAULT_ADDR must not contain credentials, query, or fragment"
        )
    loopback = parsed.hostname in {"127.0.0.1", "localhost", "::1"}
    if (
        parsed.scheme == "http"
        and not loopback
        and not _enabled("VERDIKT_VAULT_ALLOW_INSECURE_HTTP")
    ):
