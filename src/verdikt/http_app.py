...
class DashboardServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        if not _is_loopback_host(address[0]) and not api_token:
            raise ValueError(
                "VERDIKT_API_TOKEN is required when the dashboard binds outside loopback"
            )
...
def serve_dashboard(
    ...
    print(f"Verdikt dashboard listening on http://{host}:{port} ({auth_mode})", flush=True)
    try:
...
<body>
  ...
  <script>
    async function request(path, options = {}) {
      const token = window.sessionStorage.getItem('verdiktToken');
      options.headers = {...(options.headers || {})};
      if (token) options.headers.Authorization = `Bearer ${token}`;
      const response = await fetch(path, options);
      if (response.status === 401) {
        const supplied = window.prompt('Verdikt API token');
        if (supplied) {
          window.sessionStorage.setItem('verdiktToken', supplied);
