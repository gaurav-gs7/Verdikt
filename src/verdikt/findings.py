...
class ArgusAlertmanagerSink:
    ...
    def __init__(self, base_url: str, token: str, timeout_seconds: float, hmac_secret: str) -> None:
        ...
        self._token = token
        self._timeout_seconds = _positive_number(timeout_seconds, "VERDIKT_ARGUS_TIMEOUT_SECONDS")
    ...
    def send(self, event: dict[str, Any]) -> None:
        ...
        headers = {
            ...
            "User-Agent": "verdikt/1",
            "X-Verdikt-Event-ID": event["finding_id"],
        }
        if self._token:
            headers["Authorization"] = f"Bearer {self._token}"
        if self._hmac_secret:
            headers["X-Verdikt-Signature-256"] = "sha256=" + hmac.new(
...
class FindingDispatcher:
    ...
    def __init__(self, path: Path, sink: FindingSink | None = None) -> None:
        ...
        self._retry_interval = _positive_float_env(
            "VERDIKT_FINDING_RETRY_INTERVAL_SECONDS", 5.0, minimum=0.1
        )
        self._retry_base = _positive_float_env(
            "VERDIKT_FINDING_RETRY_BASE_SECONDS", 1.0, minimum=0.01
        )
...
def build_finding_event(
    *,
    ...
    dedupe_window = max(
        60, _positive_int_env("VERDIKT_FINDING_DEDUPE_WINDOW_SECONDS", 300)
    )
    dedupe_bucket = int(time.time() // dedupe_window)
    dedupe_key = _sha256(f"{server}:{tool}:{rule}:{dedupe_bucket}")
    return {
        "schema_version": 1,
        "event_type": "verdikt.mcp.security_finding",
...
def _argus_endpoint(base_url: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise ValueError("VERDIKT_ARGUS_URL must be an absolute HTTP(S) URL")
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError("VERDIKT_ARGUS_URL must not contain credentials, query, or fragment")
    loopback = parsed.hostname in {"127.0.0.1", "localhost", "::1"}
    allow_http = os.getenv("VERDIKT_ARGUS_ALLOW_INSECURE_HTTP", "").lower() in {
        "1",
        "true",
        "yes",
    }
    if parsed.scheme == "http" and not loopback and not allow_http:
        raise ValueError(
            "non-loopback Argus integration requires HTTPS or VERDIKT_ARGUS_ALLOW_INSECURE_HTTP=true"
        )
...
def _argus_payload(event: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = f"Verdikt security finding: {tool} {outcome} by {rule}"
    labels = {
        "alertname": "VerdiktMCPSecurityFinding",
        "service": f"mcp-{server}"[:63],
        "environment": _label(os.getenv("VERDIKT_ENVIRONMENT", "production"))[:63],
        "severity": severity,
        "source": "verdikt",
        "rule": rule[:63],
    }
