...
class TraceSpan:
    ...
    def set_policy(self, *, allowed: bool, rule: str, reason: str) -> None:
        self.set_attribute("verdikt.policy.allowed", allowed)
        self.set_attribute("verdikt.policy.rule", rule)
        self.set_attribute("verdikt.policy.reason", reason)
...
class Telemetry:
    """Optional OpenInference-compatible tracing for Verdikt."""

    def __init__(self, mode: str | None = None) -> None:
        self.mode = (mode or os.getenv("VERDIKT_TELEMETRY", "disabled")).lower()
        self._tracer: Any = None
        if self.mode in {"", "0", "disabled", "false", "off"}:
            self.mode = "disabled"
            return
        if self.mode not in {"console", "otlp"}:
            raise ValueError("VERDIKT_TELEMETRY must be disabled, console, or otlp")
        try:
        ...
        provider = TracerProvider(resource=Resource.create({"service.name": "verdikt"}))
