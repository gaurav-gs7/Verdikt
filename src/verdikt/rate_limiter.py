...
"""
    ...
    def __init__(self, redis_url: str, prefix: str = "verdikt:rate") -> None:
        try:
            import redis
...
def build_rate_limiter() -> RateLimiter:
    redis_url = os.getenv("VERDIKT_REDIS_URL", "")
    if not redis_url:
        return LocalRateLimiter()
    try:
        return RedisRateLimiter(redis_url)
    except RuntimeError:
        if os.getenv("VERDIKT_REDIS_REQUIRED", "").lower() in {"1", "true", "yes"}:
            raise
